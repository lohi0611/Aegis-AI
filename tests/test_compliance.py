...
class TestPPERuleEngine:
    ...
    def _make_worker(self, ppe_classes):
        return {
            ...
            "confidence": 0.85,
            "assigned_ppe": [
                {"class_name": c, "confidence": 0.80, "bbox": [0, 0, 50, 50]}
    ...
    def test_mask_required_violation(self):
        engine_with_mask = PPERuleEngine(
            require_hardhat=True, require_vest=True, require_mask=True
        )
        worker = self._make_worker(["Hardhat", "Safety Vest", "NO-Mask"])
...
class TestTemporalHysteresisFilter:
    ...
    def test_resolution_after_clean_frames(self):
        ...
        filt = TemporalHysteresisFilter(
            violation_confirm_frames=2, resolution_confirm_frames=3
        )
        viol_worker = [
            {
                "worker_id": "WKR_101",
...
class TestComplianceEngine:
    def test_no_persons_detected(self):
        ...
        detections = [
            {"class_name": "Safety Cone", "bbox": [10, 10, 50, 50], "confidence": 0.75}
        ]
    ...
    def test_full_pipeline_violation(self):
        ...
        engine = ComplianceEngine(
            require_hardhat=True, require_vest=False, violation_confirm_frames=1
        )
        detections = [
