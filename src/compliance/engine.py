...
class ComplianceEngine:
    ...
    def process_frame_detections(
        ...
        for d in detections:
            ...
            if cname == "Person":
                track_id = (
                    ...
                    if person_track_ids
                    and person_idx < len(person_track_ids)
                    and person_track_ids[person_idx]
        ...
        associated_workers, isolated_ppe = self.associator.associate(
            person_dets, ppe_dets
        )

        # 2. Rule Evaluation
        evaluated_workers = [
            self.rule_engine.evaluate_worker(w) for w in associated_workers
        ]
        ...
        standalone_violations = [
            p
            for p in isolated_ppe
            if p["class_name"] in {"NO-Hardhat", "NO-Safety Vest", "NO-Mask"}
        ]

        confirmed_worker_violations = sum(
            1 for w in stabilized_workers if w["status"] == "Violation"
        )
